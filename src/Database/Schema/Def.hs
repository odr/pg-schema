...
class
  ...
  tabFldDefs :: M.Map Text FldDef
  tabFldDefs = M.fromList $ L.zip
    (tdFlds (tabDef @sch @name))
    (fldDefs @sch @name @(TdFlds (TTabDef sch name)))
...
class
  ...
  tables :: M.Map T.Text TabDef
  tables = M.fromList $ L.zip
    (toStar @_ @(TTabs sch))
    (toStar @_ @(TTabDefs sch (TTabs sch)))

  rels :: M.Map T.Text RelDef
  rels = M.fromList $ L.zip
    (toStar @_ @(TRels sch))
    (toStar @_ @(TRelDefs sch (TRels sch)))
