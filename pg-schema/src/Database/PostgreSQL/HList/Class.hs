...
module Database.PostgreSQL.HList.Class
...
import Database.PostgreSQL.HList.Utils
...
class IsoHList ren sch (tab :: NameNSK) r where
...
instance (CFieldInfo sch tab fld, TFieldInfo sch tab fld ~ fi
  ...
  where
    type HListRepRen ren sch tab fld t =
      GHListRepFi ren sch tab fld (TFieldInfo sch tab fld) t
