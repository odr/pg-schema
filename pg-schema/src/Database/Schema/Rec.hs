...
typedPar :: FieldInfo a -> Text
typedPar fi = case fi.fieldKind of
  RFPlain fd -> fromString "?::" <> qualName fd.fdType
  _ -> fromString "?"

-- typedParArr :: forall sch a. CSchema sch => FieldInfo a -> Text
-- typedParArr fi = case fi.fieldKind of
--   RFPlain fd -> case typDefMap @sch M.!? fd.fdType of
--     Just td -> fromString "?::"
--       <> maybe (qualName fd.fdType) ((<> fromString "[]") . qualName) td.typElem
--       <> fromString "[]"
--     Nothing -> fromString "?" -- impossible!
--   _ -> fromString "?"
...
type family FromRecordInfo (r :: [(FieldInfoK NameNSK, Type)]) :: Type where
  FromRecordInfo '[ '(fi, t)] = FieldDbName fi := t
  FromRecordInfo ('(fi, t) : xs) = FieldDbName fi := t :.. FromRecordInfo xs
...
class CRecordInfo sch (tab :: NameNSK) r where
  type TRecordInfo sch tab r :: [(FieldInfoK NameNSK, Type)]
  getRecordInfo :: RecordInfo

-- Modern case: Instances was generated by AI :-)
instance (CRecordInfo sch t (PgTagged n r), ToStar t)
    => CRecordInfo sch t (HListTag '[ '(n, r) ]) where
  type TRecordInfo sch t (HListTag '[ '(n, r) ]) = TRecordInfo sch t (PgTagged n r)
  getRecordInfo = getRecordInfo @sch @t @(PgTagged n r)

instance
  ( CRecordInfo sch t (PgTagged s v)
  , CRecordInfo sch t (HListTag (f ': fs))
  , ToStar t
  ) => CRecordInfo sch t (HListTag ('(s, v) ': f ': fs)) where

  type TRecordInfo sch t (HListTag ('(s, v) ': f ': fs)) =
    TRecordInfo sch t (PgTagged s v) ++ TRecordInfo sch t (HListTag (f ': fs))

  getRecordInfo = RecordInfo
    { tabName = demote @t
    , fields = (getRecordInfo @sch @t @(PgTagged s v)).fields
            <> (getRecordInfo @sch @t @(HListTag (f ': fs))).fields
    }
