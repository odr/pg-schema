...
-- | Нам нужна категория ЭЛЕМЕНТА массива
type family GetElemCategory sch t f :: Symbol where
  -- Используем твои синглтоны: достаем TypElem, а затем его категорию
  GetElemCategory sch t f =
    TypCategory (TTypDef sch (FromJust (TypElem (GetTypDef sch t f))))
