...
-- Use 'TFieldInfo' to get 'RecFieldK' directly; use 'GetFldDef' / 'getFldDef' for plain field def.
class (ToStar name, ToStar (TFieldInfo sch tab name)) =>
      CFieldInfo sch (tab :: NameNSK) (name :: Symbol) where
  type TFieldInfo sch tab name :: RecFieldK NameNSK

genDefunSymbols [''TFieldInfo]
...
type family GetFldDef (sch :: k) (tab :: NameNSK) (name :: Symbol) :: FldDefK where
  GetFldDef sch tab name = PlainFldDef (TFieldInfo sch tab name)
...
type family GetRelDef (sch :: k) (tab :: NameNSK) (name :: Symbol) :: RelDefK where
  GetRelDef sch tab name = RelDefFromField (TFieldInfo sch tab name) tab
...
getFldDef
  :: forall sch t n. (CFieldInfo sch t n, CPlainFldDef (TFieldInfo sch t n))
  => FldDef
getFldDef = case demote @(TFieldInfo sch t n) of
  RFPlain fd -> fd
  _ -> error "impossible"
