...
promote [d|
  getRel
    :: Eq s => NameNS' s -> [(NameNS' s, RelDef' s)] -> s -> Maybe (RelDef' s)
  getRel tab@(NameNS ns _) rs s = case L.filter cmpName rs of
    [(_,x)] -> Just x
    xs -> case L.filter cmpNS xs of
      [(_,x)] -> Just x
      _       -> Nothing
    where
      isRelTab (RelDef rfrom rto _) = rfrom == tab || rto == tab
      cmpName (NameNS _ r, rd) = r == s && isRelTab rd
      cmpNS (NameNS ns' _, rd) = ns' == ns && isRelTab rd

  getRelTab
    :: Eq s => NameNS' s -> [(NameNS' s, RelDef' s)] -> s -> NameNS' s
  getRelTab tab rs s = case getRel tab rs s of
    Just rd
      | rdFrom rd == tab -> rdTo rd
      | otherwise -> rdFrom rd
    _ -> error "No relation by name"

  -- we get name of Haskell field and try to understand its kind:
  -- - if it is one of table fields then it is a plain field
  -- - it there is one constraint with such unqualified name
  --   then it is a constraint name
  -- - it there is one constraint with such qualified name
  --   then it is a constraint name
  getFldKind
    :: Eq s
    => TabDef' s -> [(NameNS' s, RelDef' s)] -> NameNS' s -> s -> FldKind' s
  getFldKind (TabDef flds _ _) rs tab s =
    case L.find (== s) flds of
      Just _ -> FldPlain
      _ -> case getRel tab rs s of
        Just x -> byRelDef x
        _      -> FldUnknown s
    where
      byRelDef rd@(RelDef rfrom rto _)
        | rfrom == tab  = FldFrom rd
        | rto == tab    = FldTo rd
        | otherwise     = FldUnknown s
...
type family
  CFldDefs (sch::Type) (tab::NameNSK) (flds::[Symbol]) :: Constraint where
  CFldDefs sch tab '[] = ()
...
--
tabDefMap :: forall sch. CSchema sch => M.Map NameNS (TabDef, M.Map Text FldDef)
...
--
relDefMap :: forall sch. CSchema sch => M.Map NameNS RelDef
...
type TRelTab sch t name = GetRelTab t (TRelDefs sch) name
