...
data Cond (sch::Type) (tab::NameNSK) where
  ...
  Cmp :: forall fld v sch tab.
    ( CDBFieldInfo sch tab fld, KnownSymbol fld, ToField v, Show v
    , CanConvert sch tab fld v)
    => Cmp -> v -> Cond sch tab
  In :: forall fld v sch tab.
    ( CDBFieldInfo sch tab fld, KnownSymbol fld, SingI (TDBFieldInfo sch tab fld)
    , CPlainFldDef (TDBFieldInfo sch tab fld), ToField v, Show v
    , CanConvert sch tab fld v )
  ...
  Null :: forall fld sch tab.
    (CDBFieldInfo sch tab fld, KnownSymbol fld
...
pnull :: forall sch tab. forall name ->
  (CDBFieldInfo sch tab name, KnownSymbol name, FdNullable (GetFldDef sch tab name) ~ 'True) =>
  Cond sch tab
...
pin :: forall name -> forall sch tab v.
  ( CDBFieldInfo sch tab name, CPlainFldDef (TDBFieldInfo sch tab name)
  , Show v, ToField v, KnownSymbol name, SingI (TDBFieldInfo sch tab name)
...
data OrdFld sch tab where
  OrdFld :: forall fld sch tab. (CDBFieldInfo sch tab fld, KnownSymbol fld) =>
...
{-# INLINE ordf #-}
ordf
  :: forall fld -> forall sch tab. (CDBFieldInfo sch tab fld, KnownSymbol fld)
  => OrdDirection -> OrdFld sch tab
ordf fld = OrdFld @fld
