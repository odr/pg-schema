...
| API | Вход (compile-time) | SQL |
|-----|---------------------|-----|
| `insertSch` | все mandatory | `INSERT` |
| `updateByKey` | полный PK или подходящий UK | `UPDATE` (без insert) |
| `upsertByKey` | mandatory **и** полный PK/UK | `INSERT … ON CONFLICT …` |
...
| `upsert*` | если на узле входа есть все mandatory — returning без `Maybe`; иначе на каждой list-ветке элемент **обязан** быть `Maybe`; лишний `Maybe` при полном узле — ошибка компиляции |
| `update*` | на каждой list-ветке returning **обязан** быть `Maybe`; bare `t` **запрещён** |
